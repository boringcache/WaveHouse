...
- **`policy/`** — Hasura-style access control: `Policy`/`TablePolicy`/`RolePermissions` types, `Evaluate()` engine with JWT claim templating, `Source` (a `func() *Policy` read per call; `settings.Store.Policy` in production, `Static(p)` in tests)
