...
- **compact.go** — `EncodeCompactRow`, the transitional positional row encoder. Inert today: it has no non-test caller, and exists so the wire-format change can land separately from the code that adopts it.
